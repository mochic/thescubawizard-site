...
export default ({ show }) => {
  ...
  const h2Props = useSpring({
    from: {
      h2Transform: `translate3d(0px,0,0)`,
      h2Opacity: 0,
      filter: `blur(20px)`,
    },
    to: {
  ...
  const proposalSpringRef = useRef()
  const { pTransform, pOpacity } = useSpring({
    from: {
      pTransform: `translate3d(0,40px,0)`,
      pOpacity: 0,
    },
    to: { pTransform: `translate3d(0,0px,0)`, pOpacity: 1 },
    ref: proposalSpringRef,
  })
